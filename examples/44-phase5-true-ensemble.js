...
// ═══════════════════════════════════════════════════════════════════
//   TRUE ENSEMBLE: TRAIN 3 INDEPENDENT PHASE 5 MODELS
// ═══════════════════════════════════════════════════════════════════
//
// APPROACH: Train multiple models with different random initialization
//
// Key Differences from Phase 5:
// 1. Train 3 separate models with different random seeds
// 2. Each model learns slightly different patterns
// 3. Consensus predictions boost effective confidence
// 4. Only trade when 3/3 models agree above threshold
//
// Expected Results:
// - Individual models: 40-50% confidence (like Phase 5)
// - Consensus signals: 55-70% effective confidence
// - Fewer signals but much higher quality
//
// Time: ~2-3 hours (3 models × 40-45 min each)
// ═══════════════════════════════════════════════════════════════════
...
console.log('═'.repeat(71));
console.log('   PHASE 5 TRUE ENSEMBLE: TRAINING 3 INDEPENDENT MODELS');
console.log('═'.repeat(71));
console.log('');
console.log(`Backend: ${tf.getBackend().toUpperCase()}`);
console.log('');
console.log('Strategy: Train 3 models with different random seeds');
console.log('          → Each learns slightly different patterns');
console.log('          → Consensus predictions boost confidence');
console.log('          → Only trade when 3/3 models agree');
console.log('');
console.log('Expected time: 2-3 hours (3 models × 40-45 min each)');
console.log('');
...
console.log(`  Number of models: ${NUM_MODELS}`);
...
for (let i = 1; i <= NUM_MODELS; i++) {
  const result = await trainModel(i, NUM_MODELS, allData, FORWARD_DAYS, TOP_PERCENTILE);
...
for (const result of modelResults) {
  ...
  avgAccuracy += result.testAccuracy;
  avgTime += result.trainingTime;
}

avgAccuracy /= NUM_MODELS;
avgTime /= NUM_MODELS;

console.log('─'.repeat(71));
console.log(`Average Accuracy: ${(avgAccuracy * 100).toFixed(1)}%`);
console.log(`Average Time: ${avgTime.toFixed(1)} min per model`);
console.log('');
console.log('Next step: Run ensemble predictor with consensus voting');
console.log('          → node examples/45-phase5-ensemble-predict.js');
console.log('');
console.log('═'.repeat(71));
console.log('');
