...
// ═══════════════════════════════════════════════════════════════════
//   PHASE 6: 10-DAY TRENDS + TOP 15% FOR ULTRA-HIGH CONFIDENCE
// ═══════════════════════════════════════════════════════════════════
//
// EVOLUTION from Phase 5:
// Phase 5 achieved 41.6% average confidence with 5-day / top 30%.
// To reach 65%+ confidence, we need more selective predictions.
//
// NEW APPROACH:
// - Predict 10-day forward returns (longer trends, less noise)
// - Target: Top 15% performers (elite momentum stocks)
// - This creates even clearer signals with higher confidence
//
// Expected improvements:
// - Confidence scores in 60-75% range
// - Highly selective signals (only the strongest setups)
// - Better risk/reward on each trade
// - More suitable for real trading with capital at risk
//
// ═══════════════════════════════════════════════════════════════════
...
console.log('═══════════════════════════════════════════════════════════════════');
console.log('   PHASE 6: 10-DAY TRENDS + TOP 15% (ULTRA-HIGH CONFIDENCE)');
console.log('═══════════════════════════════════════════════════════════════════');
console.log('');
...
// Generate enhanced features with 10-day forward returns
console.log('\n[2] Generating enhanced features with 10-day returns...');
const enhancedData = {};
let totalSamples = 0;
let processedSymbols = 0;

const FORWARD_DAYS = 10;  // Predict 10 days ahead (longer trends)
const TOP_PERCENTILE = 0.15;  // Top 15% performers (elite signals)
...
for (const [symbol, bars] of Object.entries(allData)) {
  ...
  // Calculate 10-day forward returns for all samples
  ...
  // Calculate threshold for top 15% returns
  if (validReturns.length > 0) {
    ...
    // Label: 1 if in top 15% of returns, 0 otherwise
...
await model.save('file://./models/phase6-10day-top15');
fs.writeFileSync('./models/phase6-normalization.json', JSON.stringify({
  means,
  stds,
  featureNames,
  totalSamples,
...
console.log('✓ Model saved to ./models/phase6-10day-top15');
...
console.log('✓ Phase 6 training complete!');
console.log('');
